...
label1=customtkinter.CTkLabel(master=frame1,text='Apple health analyzer')
...
combobox = customtkinter.CTkComboBox(master=frame2,
                                     values=d,
...
# button31=customtkinter.CTkButton(master=frame3,text='show graph',command=(lambda:steps()))
...
label3.pack(pady=12,padx=10)
d=code.datestep()
combobox_varrr = customtkinter.StringVar(value=d[0])  # set initial value

def combobox_callback(choice):
    print("combobox dropdown clicked:", choice)

comboboxx = customtkinter.CTkComboBox(master=frame3,
                                     values=d,
                                     command=combobox_callback,
                                     variable=combobox_var)
comboboxx.pack(padx=20, pady=10)
print(comboboxx.get())
button31=customtkinter.CTkButton(master=frame3,text='show graph',command=(lambda:code.createstepsfile(comboboxx.get())))
button31.pack(pady=12,padx=10)
button3=customtkinter.CTkButton(master=frame3,text='home',command=lambda:raise_frame(frame1))
button3.pack(pady=12,padx=10)#2021-10-03
checkbox3=customtkinter.CTkCheckBox(master=frame3,text='its working')
